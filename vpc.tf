...
resource "aws_internet_gateway" "craftcms_igw" {
  vpc_id = aws_vpc.craftcms-vpc.id
...
resource "aws_route" "public-internet-gateway-route" {
  route_table_id = aws_route_table.public-route-table.id
  gateway_id = aws_internet_gateway.craftcms_igw.id
  destination_cidr_block    = "0.0.0.0/0"
...
resource "aws_security_group" "craftcms" {
  name        = "craftcms"
